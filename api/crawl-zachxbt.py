...
class ZachXBTSpider(Spider):
    ...
    def parse(self, response):
        ...
        for thread_card in response.xpath('//div[@data-link-href]'):
            thread_url = thread_card.xpath('@data-link-href').get()
            date = thread_card.xpath('div[contains(@class, "thread-card")]/div[contains(@class, "thread-info")]//text()').get()
            if not re.search(r',', date):
                date = date + ", 2025"
            
            title = "Investigation: " + str(date)
            sanitized_url = response.urljoin(thread_url);
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        df = pd.DataFrame(ZachXBTSpider.results)
